...
public class LogicServerUtil
{
    ...
    {
        while (!m_tcpClient.Connect(NetConfig.s_logicService_ip, (ushort)NetConfig.s_logicService_port, false))
        {
            // 连接数据库服务器失败的话会一直尝试连接
            LogUtil.getInstance().addDebugLog("连接逻辑服务器失败");
            Thread.Sleep(1000);
        }

        m_isConnecting = true;
        LogUtil.getInstance().addDebugLog("连接逻辑服务器成功");
    ...
    public void stop()
    {
        if (m_tcpClient.Stop())
        {
            LogUtil.getInstance().writeLogToLocalNow("与逻辑服务器断开成功");
            m_isConnecting = false;
        }
        else
        {
            LogUtil.getInstance().writeLogToLocalNow("与逻辑服务器断开失败");
        }
    }
    ...
    public bool sendMseeage(string str)
    {
        ...
        try
        {
            ...
            {
                LogUtil.getInstance().addDebugLog("发送给逻辑服务器成功:" + str);
                return true;
            }
            else
            {
                LogUtil.getInstance().addDebugLog("发送给逻辑服务器失败:" + str);
                return false;
            }
        }
        catch (Exception ex)
        {
            LogUtil.getInstance().addDebugLog("发送给逻辑服务器异常:" + ex.Message);
            return false;
        }
    }
    ...
    HandleResult OnSend(TcpClient sender, byte[] bytes)
    {
        ...
        //LogUtil.getInstance().addDebugLog("发送给逻辑服务器:" + str);
    ...
    {
        string str = Encoding.UTF8.GetString(bytes, 0, bytes.Length);
        LogUtil.getInstance().addDebugLog("收到逻辑服务器消息:" + str);
    ...
    {
        LogUtil.getInstance().addDebugLog("与逻辑服务器断开");
